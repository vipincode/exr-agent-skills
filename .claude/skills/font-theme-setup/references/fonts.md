# Fonts: wiring Figma typography into Next.js (`next/font`)
...
For licensed or bespoke fonts shipped as files. Put the files under `src/app/fonts/` (e.g.
`.woff2`) and load them locally.
...
Add `brand.variable` to the `<html className>` alongside the others. Prefer `.woff2` (smallest);
if you only have `.ttf`/`.otf` they work but are larger.
...
## Mapping Figma → font roles

| Figma usage | Role / variable | Utility |
| --- | --- | --- |
...
If Figma uses one family throughout, just set `--font-sans` and skip the rest. If it uses a
distinct display face for headings, give that its own variable so headings and body stay
separable. Get the family names from `get_variable_defs` (`font/family/*`) or the text layers in
`get_design_context`.

## Checklist

- [ ] Every Figma family is declared in `layout.tsx` (google or local), with the right weights.
...
- [ ] No `<link rel="stylesheet">` to Google Fonts and no `@import url()` left behind.
