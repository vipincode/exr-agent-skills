...
6. **Assets.** For `<img>`/background images/icons in the HTML: copy local asset files into `public/` (or the feature) and use `next/image`; for remote `src` URLs, download into `public/` rather than hotlinking. Inline small/recolorable SVGs as components (`currentColor`). See `references/building-components.md` ("Assets").
...
- **shadcn primitives are composed, never duplicated.** Need a carousel/avatar/chart/tabs/sidebar? `shadcn add` it and wrap it. Hand-writing a primitive shadcn ships, or re-skinning `Button`/`Select`, is a regression.
