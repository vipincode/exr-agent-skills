# Reading the backend — the resolution ladder

The point of this skill is that the binding plan is built on the **real** API contract, not a
guess. Take the **highest-fidelity source you can reach** and stop there. Record which rung you
used in the plan's *API contract (observed)* section so the user (and `frontend-module-builder`)
can trust it.

The single fact that matters most is the **success envelope** — the exact JSON the endpoint
returns. The frontend types, Zod schemas, and BFF route all mirror it. A wrong envelope is the
...
- If a single fact is unavailable on every rung you can reach (e.g. the exact pagination meta),
  **do not invent it** — list it under Open questions and let the user fill it in. A flagged gap
  is recoverable; a silently-wrong type is not.
